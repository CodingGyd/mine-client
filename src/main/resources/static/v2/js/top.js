		        	...
		        	if (null != result) {
		        		...
		        		for(var i=0;i<result.length;i++){
		        			if(result[i].dm == null || result[i].dm == '') {
		        				if ((null == type_dm || '' == type_dm) && (null != limit && '' != limit )) {
			        				html +=  '<li class="menu"><a  id="selected" href="/list?page=1&amp;limit=10" >'+result[i].ms+'</a><ul class="sub">';
		        		...
		        		for (var i=0;i<result.length;i++) {
		        			if (i == rootIndex) continue;
		        				...
		        				if (type_dm == result[i].dm) {
			        				html += '<li><a  id="selected"  href="/list?page=1&amp;limit=10&amp;type_dm='+result[i].dm+'"    >'+result[i].ms+'</a></li>';
			        				$(document).attr("title",result[i].ms);
		        				} else {
